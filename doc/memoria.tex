...
En la gráfica \ref{fig:higgs,_curvas_de_aprendizaje} podemos ver cómo tanto el \textit{Random Forest} como el \textit{KNN} están sobreajustando los datos. El \textit{SVC} parece que mejora al utilizar más datos, incrementando en 0.1 su valor F1 desde el 10\% de los datos hasta el 100\% de ellos. Tanto el \textit{Logistic Regression} como el \textit{Gaussian NB} no mejoran demasiado con más datos, así que utilizaremos sólo el 10\% de ellos, consiguiendo una notable aceleración en el tiempo de cálculo. Si tras la validación cruzada vemos que el \textit{Random Forest} o el \textit{KNN} dejan de sobreajustar, o que el \textit{SVC} generaliza bien los datos, nos plantearemos la posibilidad de usar todos los datos para el entrenamiento de los algoritmos.

Tras el proceso de validación cruzada sobre el 10\% de los datos obtenemos las métricas de la tabla \ref{tab:HiggsMetricas}. Los mejores parámetros encontrados son: para el \textit{SVC}, \textit{kernel} lineal con $C=1000$ y $\gamma = 0.1$; para el \textit{Logistic Regression} $C=100$; para el \textit{Random Forest} 15 árboles, con un mínimo de 10 muestras para separar una rama, y con un mínimo de 4 muestras por hoja; para el \textit{KNN}, 5 vecinos. Algunos de estos parámetros son los extremos de la malla inicial que estamos utilizando, por ejemplo el valor de $C$ del \textit{SVC}, el número de árboles o las muestras mínimas para separar una rama del \textit{Random Forest}, lo cual nos anima a volver a hacer una validación cruzada con una malla más ``fina'' en torno a estos parámetros. Además, estos dos algoritmos son los que mejores métricas han dado, merece la pena perder un poco de tiempo en tratar de mejorarlos.
	...
	\centering
		\begin{tabular}{ccccc}
		  \toprule
			Algoritmo & Accuracy & Precision & Recall & F1-Score \\
			\midrule
			\textit{Gaussian NB} & 0.6858 & 0.7367 & 0.8144 & 0.7736 \\
			\textit{SVC} & 0.8129 & 0.8413 & 0.8826 & 0.8614 \\
			\textit{Logistic Regression} & 0.7412 & 0.7810 & 0.8608 & 0.8190 \\
			\textit{Random Forest} & 0.8252 & 0.8450 & 0.9000 & 0.8716 \\
			\textit{KNN} & 0.7249 & 0.7855 & 0.8015 & 0.7934 \\
			\bottomrule
			\end{tabular}
...
La nueva malla que probamos para estos algoritmos es así: para el \textit{SVC}, con el \textit{kernel} rbf, $\gamma \in \{0.01, 0.03, 0.1, 0.3, 1.0\}$ y $C\in\{100, 300, 1000, 3000, 10000\}$; mientras que para el \textit{Random Forest} usamos 10, 13, 15, 17 y 20 árboles, con 8, 9, 10 y 11 como mínimo de muestras para separar una rama, y con 2, 3, 4, 5 y 6 como mínimo de muestras por hoja. 

Los nuevos parámetros encontrados son, para el \textit{SVC}, $C=10000$y $\gamma=0.03$, y para el \textit{Random Forest}, 20 árboles, con 11 muestras para separar una rama y con 5 muestras como mínimo por hoja. Las métricas conseguidas por estos algoritmos se recogen en la tabla \ref{tab:HiggsNuevasMetricas}.
	...
	\centering
		\begin{tabular}{ccccc}
		  \toprule
			Algoritmo & Accuracy & Precision & Recall & F1-Score \\
			\midrule
			\textit{SVC} & 0.8133 & 0.8408 & 0.8842 & 0.8619 \\
			\textit{Random Forest} & 0.8270 & 0.8476 & 0.8993 & 0.8727 \\
			\bottomrule
			\end{tabular}

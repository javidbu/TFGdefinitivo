...
\begin{equation}
J'(\theta) = J(\theta) + \frac{1}{C}\sum_j{\theta_j^2}
\label{eq:ErrorRegul}
\end{equation}
...
Sea $x$ el vector columna de variables de entrada, y $\theta$ el vector columna de pesos, ambos de la misma dimensión. Por convenio, al vector $x$ le añadimos una primera componente, $x_0 = 1$, que tiene su peso correspondiente, $\theta_0$. Esta componente añade un término constante a la combinación lineal de las variables de entrada, como ahora veremos. La hipótesis del algoritmo es \eqref{eq:hipLogReg}, donde $g(z)$ es la función logística de \eqref{eq:sigmoid}. Ésta se interpreta como la probabilidad de que, dados el vector $x$ y los pesos $\theta$, la etiqueta correspondiente a esos datos sea ``1''.

\begin{equation}
\encuadre{h_\theta(x) = p(y = 1 | x; \theta) = g(\theta^T x) = \frac{1}{1+e^{-\theta^T x}}}
\label{eq:hipLogReg}
\end{equation}

Por norma general, predeciremos ``$y=1$'' si $h_\theta(x)>0.5$, es decir (ver gráfica \ref{fig:sigmoid}), si $\theta^T x > 0$, por lo que nuestra superficie de decisión, lineal en las variables $x$, será $\theta^T x = 0$.
...
\begin{equation}
\encuadre{J(\theta) = -\frac{1}{m}\sum_{i=1}^{m}{\left[y^{(i)}\log{\left(h_\theta(x^{(i)})\right)} + \left(1-y^{(i)}\right)\log{\left(1-h_\theta(x^{(i)})\right)}\right]}}
\label{eq:errLogReg}
\end{equation}
...
\caption{Función de error frente a $h_\theta(x)$ para los casos $y=1$ y $y=0$.}
\label{fig:errLogReg}
\end{figure}
...
\begin{equation}
p(\bm{x}|C) = \prod_{i=1}^n{p(x_i|C)}
\label{eq:NaiveBayes}
\end{equation}
donde $\bm{x}$ es el vector de variables, cuyas componentes son $x_i$, y $C$ es una determinada clase (en nuestros problemas de clasificación binaria, $C$ será 0 ó 1). 

Gracias al teorema de Bayes, sabemos que la probabilidad de que dado el vector de variables $\bm{x}$ se de la clase $C_j$ es:
\begin{equation}
p(C_j|\bm{x}) = \frac{p(C_j)p(\bm{x}|C_j)}{p(\bm{x})}
\label{eq:TeoremaBayes}
\end{equation}

En general, para un problema de clasificación multiclase, siendo $C_j$ cada una de las clases, el algoritmo asignará a cada entrada nueva de datos la clase $C_k$ que maximice la probabilidad de la ecuación \ref{eq:TeoremaBayes}. Ésta es la hipótesis del algoritmo. Nótese que, dado un vector $\bm{x}$, $p(\bm{x})$ es una constante, por lo que sólo tenemos que preocuparnos por maximizar el numerador, teniendo en cuenta \eqref{eq:NaiveBayes}.

 Para calcular las probabilidades $p(x_i|C)$, hay que tener en cuenta el tipo de distribución que sigue la variable $i$, dando lugar a diferentes algoritmos, como por ejemplo, el ``Naive Bayes Gaussiano'', el ``Naive Bayes Multinomial'' o el ``Naive Bayes Binomial'', todos ellos nombrados según la distribución de probabilidad de las variables. También se pueden hacer algoritmos de Naive Bayes mixtos, donde a distintas variables les correspondan distintas distribuciones de probabilidad.

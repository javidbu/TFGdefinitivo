...
\newcommand{\titulo}{Algoritmos de Aprendizaje Autom\'atico y sus aplicaciones}
...
La mayoría de los algoritmos de \ML  dependen de parámetros que hay que fijar, optimizando las métricas conseguidas por el algoritmo.% Por ejemplo, muchos de ellos utilizan el \textit{parámetro de smoothing}, $C$, que veremos a continuación. El problema es que los valores de estos parámetros influyen en gran medida en la bondad de la clasificación, y una mala elección de los parámetros nos puede hacer pensar que un algoritmo es malo, cuando la realidad es que lo estamos utilizando mal. 
Hasta ahora el procedimiento a seguir era entrenar el algoritmo con el conjunto de aprendizaje, y ver la bondad del mismo con el conjunto de test. Ahora, para fijar correctamente estos parámetros, usaremos un tercer conjunto, el conjunto de validación cruzada (\textit{Cross Validation set}. Los pasos a seguir ahora son los siguientes:
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.50\textwidth]{graficas/sigmoid}
...
\begin{figure}[htbp]
	\centering
		...
		\subfigure[1 vecino, \textit{overfitting}]{\includegraphics[width=0.40\textwidth]{graficas/KNN1Overfit}}
		\subfigure[3 vecinos, ya no hay \textit{overfitting}]{\includegraphics[width=0.40\textwidth]{graficas/KNN3}}
	\label{fig:KNN}
...
En la primera base de datos vimos que sólo éramos capaces de conseguir valores F1 en torno a 73-76\%. Ésto se debía a que tenemos pocas variables a estudiar, y además las clases no seguían una distribución clara, siendo más difícil diferenciarlas. Hay que tener en cuenta que el ML no es un truco de magia, son algoritmos matemáticos, así que si en una base de datos éstos no contienen mucha información, un algoritmo de ML no podrá extraerla de donde no existe. Pongamos el ejemplo de un dado perfectamente equilibrado. Si alimentamos a cualquier algoritmo con los números han salido en las últimas 100000 tiradas, éste no podrá predecir qué saldrá en las siguientes, aunque sepa datos como quién tira el dado, qué temperatura hace o hacia dónde lo tira. Los algoritmos jamás podrán identificar patrones que no se encuentren en los datos.

Una vez aclarado esto, recordamos que en esta base de datos el algoritmo que mejor funcionó fue el SVM, con un valor F1 del 73\%. Para elegir éste algoritmo tuvimos que descartar el RF, con un mejor valor F1 (76\%), pero sobreajustaba los datos del conjunto de aprendizaje, así que no nos podíamos fiar de su capacidad de generalización.
...
Vimos cómo el RF acertaba al escoger las variables más importantes, siendo la masa estimada la mejor. Al final, decidimos que el mejor algoritmo para esta base de datos, al nivel de profundidad al que la estudiamos, era el SVM, aunque seguramente, si hubiéramos seguido probando mallas más finas, podría haber sido el RF.

De este trabajo podemos concluir que no existe un algoritmo mejor que el resto, sino que cada base de datos tiene ``su algoritmo'', uno con el que es más fácil sacar toda la información posible de los datos. También concluimos, gracias a la base de datos del Titanic, que donde no hay información no se puede sacar, que los algoritmos no inventan información, sólo tratan de sacarla de los datos. \textbf{He quitado el apartado de los dígitos, pero ahora el mejor algoritmo para las dos bases de datos es el SVM... Esto choca con lo que digo de que cada base de datos tiene su algoritmo... Vuelvo a poner los digitos?}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Apéndices}
\subsection{Variables del Higgs}\label{sec:features}
A continuación copio literalmente de \cite{higgsDoc} la explicación de cada una de las variables del problema del apartado \ref{sec:higgs}. 

\begin{enumerate}
	\item \texttt{EventId} \textit{An unique integer identifier of the event. Not to be used as a feature.}
	\item \texttt{DER\_mass\_MMC} \textit{The estimated mass} $m_H$ \textit{of the Higgs boson candidate, obtained through a probabilistic phase space integration (may be undefined if the topology of the event is too far from the expected topology).}
	\item \texttt{DER\_mass\_transverse\_met\_lep} \textit{The transverse mass between the missing transverse energy and the lepton.}
	\item \texttt{DER\_mass\_vis} \textit{The invariant mass of the hadronic tau and the lepton. }
	\item \texttt{DER\_pt\_h} \textit{The modulus of the vector sum of the transverse momentum of the hadronic tau, the lepton, and the missing transverse energy vector.}
	\item \texttt{DER\_deltaeta\_jet\_jet} \textit{The absolute value of the pseudorapidity separation between the two jets (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1). }
	\item \texttt{DER\_mass\_jet\_jet} \textit{The invariant mass of the two jets (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1). }
	\item \texttt{DER\_prodeta\_jet\_jet} \textit{The product of the pseudorapidities of the two jets (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1).}
	\item \texttt{DER\_deltar\_tau\_lep} \textit{The R separation between the hadronic tau and the lepton. }
	\item \texttt{DER\_pt\_tot} \textit{The modulus of the vector sum of the missing transverse momenta and the transverse momenta of the hadronic tau, the lepton, the leading jet (if \texttt{PRI\_\-jet\_\-num} = 1) and the subleading jet (if \texttt{PRI\_\-jet\_\-num} = 2) (but not of any additional jets).}
	\item \texttt{DER\_sum\_pt} \textit{The sum of the moduli of the transverse momenta of the hadronic tau, the lepton, the leading jet (if \texttt{PRI\_\-jet\_\-num} = 1) and the subleading jet (if \texttt{PRI\_\-jet\_\-num} = 2) and the other jets (if \texttt{PRI\_\-jet\_\-num} = 3). }
	\item \texttt{DER\_pt\_ratio\_lep\_tau} \textit{The ratio of the transverse momenta of the lepton and the hadronic tau. }
	\item \texttt{DER\_met\_phi\_centrality} \textit{The centrality of the azimuthal angle of the missing transverse energy vector w.r.t. the hadronic tau and the lepton
	...
	\item \texttt{DER\_lep\_eta\_centrality} \textit{The centrality of the pseudorapidity of the lepton w.r.t. the two jets (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1)
	...
	\item \texttt{PRI\_tau\_pt} \textit{The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the hadronic tau. }
	\item \texttt{PRI\_tau\_eta} \textit{The pseudorapidity $\eta$ of the hadronic tau. }
	\item \texttt{PRI\_tau\_phi} \textit{The azimuth angle $\phi$ of the hadronic tau.}
	\item \texttt{PRI\_lep\_pt} \textit{The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the lepton (electron or muon). }
	\item \texttt{PRI\_lep\_eta} \textit{The pseudorapidity $\eta$ of the lepton. }
	\item \texttt{PRI\_lep\_phi} \textit{The azimuth angle $\phi$ of the lepton.}
	\item \texttt{PRI\_met} \textit{The missing transverse energy $\vec{E}_T^{miss}$. }
	\item \texttt{PRI\_met\_phi} \textit{The azimuth angle $\phi$ of the missing transverse energy.}
	\item \texttt{PRI\_met\_sumet} \textit{The total transverse energy in the detector.}
	\item \texttt{PRI\_jet\_num} \textit{The number of jets (integer with value of 0, 1, 2 or 3; possible larger values have been caped at 3).}
	\item \texttt{PRI\_jet\_leading\_pt} \textit{The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the leading jet, that is the jet with largest transverse momentum (undefined if \texttt{PRI\_\-jet\_\-num} = 0).}
	\item \texttt{PRI\_jet\_leading\_eta} \textit{The pseudorapidity $\eta$ of the leading jet (undefined if \texttt{PRI\_\-jet\_\-num} = 0).}
	\item \texttt{PRI\_jet\_leating\_phi} \textit{The azimuth angle $\phi$ of the leading jet (undefined if \texttt{PRI\_\-jet\_\-num} = 0). }
	\item \texttt{PRI\_jet\_subleading\_pt} \textit{The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the subleading jet, that is the jet with the second largest transverse momentum (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1).}
	\item \texttt{PRI\_jet\_subleading\_eta} \textit{The pseudorapidity $\eta$ of the subleading jet (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1). }
	\item \texttt{PRI\_jet\_subleading\_phi} \textit{The azimuth angle $\phi$ of the subleading jet (undefined if \texttt{PRI\_\-jet\_\-num} $\leq$ 1).}
	\item \texttt{PRI\_jet\_all\_pt} \textit{The scalar sum of the transverse momentum of all the jets of the events. }
	\item \texttt{Label} \textit{The event label (string) $y_i\in {s,b}$ (s for signal, b for background). Not to be used as a feature.}
\end{enumerate}

Todos los ángulos azimutales $\phi$ están en radianes en el intervalo $[-\pi,\pi)$, las masas, energías y momentos están en GeV, y el resto de variables son adimensionales. Las variables que estén indefinidas, ya sea porque no tienen sentido físico o porque no han podido ser calculadas, tienen un valor de -999.0, que está fuera del rango de valores normal de todas las variables. 

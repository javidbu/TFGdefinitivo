...
Por norma general, predeciremos ``$y=1$'' si $h_{\bm{\theta}}(\bm{x})>0.5$, es decir (ver gráfica \ref{fig:sigmoid}), si $\bm{\theta}^T \bm{x} > 0$, por lo que nuestra superficie de decisión, lineal en las variables $x$, será $\bm{\theta}^T \bm{x} = 0$.

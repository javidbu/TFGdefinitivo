...
\begin{equation}
\begin{aligned}
	\bm{\theta}^T\bm{x}^{(i)} + b = +1 \text{ si } y^{(i)}& = +1 \\
	\bm{\theta}^T\bm{x}^{(i)} + b = -1 \text{ si } y^{(i)}& = -1 
\end{aligned}
\label{eq:SVMMargen}
\end{equation}
...
\begin{equation}
y^{(i)}\left(\bm{\theta}^T\bm{x}^{(i)} + b\right) \geq 1 \ \ \forall i = 1,...,m
\label{eq:SVMMargen2}
\end{equation}
Los puntos que cumplen que $y^{(i)}\left(\bm{\theta}^T\bm{x}^{(i)} + b\right) = 1$ se llaman \textit{vectores de soporte}, y son los que dan el nombre al algoritmo. Nuestra hipótesis será
\begin{equation}
\encuadre{h_{\bm{\theta}}(\bm{x}) = \text{signo}(\bm{\theta}^T\bm{x} + b)}
\label{eq:SVMHip}
\end{equation}

\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.80\textwidth]{graficas/highMargin2}
	\caption{Hiperplano de separación (b), con ecuación $\{\bm{x} \in \mathbb{R}^n:\bm{\theta}^T\bm{x} + b = 0\}$ e hiperplanos soporte, (a) con ecuación $\{\bm{x} \in \mathbb{R}^n:\bm{\theta}^T\bm{x} + b = +1\}$ y (c) con ecuación $\{\bm{x} \in \mathbb{R}^n:\bm{\theta}^T\bm{x} + b = -1\}$. Los vectores de soporte son los más cercanos al hiperplano de separación.}
...
\text{sujeto a  }& y^{(i)}\left(\bm{\theta}^T\bm{x}^{(i)} + b\right) \geq 1 \ \ \forall i = 1,...,m
\end{aligned}}

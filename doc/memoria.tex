...
\begin{enumerate}
	...
	\item \texttt{DER_mass_MMC} The estimated mass $m_H$ of the Higgs boson candidate, obtained through a probabilistic phase space integration (may be undefined if the topology of the event is too far from the expected topology).
	\item \texttt{DER_mass_transverse_met_lep} The transverse mass between the missing transverse energy and the lepton.
	\item \texttt{DER_mass_vis} The invariant mass of the hadronic tau and the lepton. 
	\item \texttt{DER_pt_h} The modulus of the vector sum of the transverse momentum of the hadronic tau, the lepton, and the missing transverse energy vector.
	\item \texttt{DER_deltaeta_jet_jet} The absolute value of the pseudorapidity separation between the two jets (undefined if \texttt{PRI_jet_num} $\leq$ 1). 
	\item \texttt{DER_mass_jet_jet} The invariant mass of the two jets (undefined if \texttt{PRI_jet_num} $\leq$ 1). 
	\item \texttt{DER_prodeta_jet_jet} The product of the pseudorapidities of the two jets (undefined if \texttt{PRI_jet_num} $\leq$ 1).
	\item \texttt{DER_deltar_tau_lep} The R separation between the hadronic tau and the lepton. 
	\item \texttt{DER_pt_tot} The modulus of the vector sum of the missing transverse momenta and the transverse momenta of the hadronic tau, the lepton, the leading jet (if \texttt{PRI_jet_num} $\=$ 1) and the subleading jet (if \texttt{PRI_jet_num} $\=$ 2) (but not of any additional jets)
	\item \texttt{DER_sum_pt} The sum of the moduli of the transverse momenta of the hadronic tau, the lepton, the leading jet (if \texttt{PRI_jet_num} $\=$ 1) and the subleading jet (if \texttt{PRI_jet_num} $\=$ 2) and the other jets (if \texttt{PRI_jet_num} $\=$ 3). 
	\item \texttt{DER_pt_ratio_lep_tau} The ratio of the transverse momenta of the lepton and the hadronic tau. 
	\item \texttt{DER_met_phi_centrality} The centrality of the azimuthal angle of the missing transverse energy vector w.r.t. the hadronic tau and the lepton
	...
	\item \texttt{DER_lep_eta_centrality} The centrality of the pseudorapidity of the lepton w.r.t. the two jets (undefined if \texttt{PRI_jet_num} $\leq$ 1)
	...
	\item \texttt{PRI_tau_pt} The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the hadronic tau. 
	\item \texttt{PRI_tau_eta} The pseudorapidity $\eta$ of the hadronic tau. 
	\item \texttt{PRI_tau_phi} The azimuth angle $\phi$ of the hadronic tau.
	\item \texttt{PRI_lep_pt} The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the lepton (electron or muon). 
	\item \texttt{PRI_lep_eta} The pseudorapidity $\eta$ of the lepton. 
	\item \texttt{PRI_lep_phi} The azimuth angle $\phi$ of the lepton.
	\item \texttt{PRI_met} The missing transverse energy $\vec{E}_T^{miss}$. 
	\item \texttt{PRI_met_phi} The azimuth angle $\phi$ of the missing transverse energy.
	\item \texttt{PRI_met_sumet} The total transverse energy in the detector.
	\item \texttt{PRI_jet_num} The number of jets (integer with value of 0, 1, 2 or 3; possible larger values have been caped at 3).
	\item \texttt{PRI_jet_leading_pt} The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the leading jet, that is the jet with largest transverse momentum (undefined if \texttt{PRI_jet_num} = 0).
	\item \texttt{PRI_jet_leading_eta} The pseudorapidity $\eta$ of the leading jet (undefined if \texttt{PRI_jet_num} = 0).
	\item \texttt{PRI_jet_leating_phi} The azimuth angle $\phi$ of the leading jet (undefined if \texttt{PRI_jet_num} = 0). 
	\item \texttt{PRI_jet_subleading_pt} The transverse momentum $\sqrt{p_x^2+p_y^2}$ of the subleading jet, that is the jet with the second largest transverse momentum (undefined if \texttt{PRI_jet_num} $\leq$ 1).
	\item \texttt{PRI_jet_subleading_eta} The pseudorapidity $\eta$ of the subleading jet (undefined if \texttt{PRI_jet_num} $\leq$ 1). 
	\item \texttt{PRI_jet_subleading_phi} The azimuth angle $\phi$ of the subleading jet (undefined if \texttt{PRI_jet_num} $\leq$ 1).
	\item \texttt{PRI_jet_all_pt} The scalar sum of the transverse momentum of all the jets of the events. 
